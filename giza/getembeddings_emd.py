...
def calculate_emd(hidict, endict, ensentence, hindisentence, probdict, idf_dict, idf_dict_hi):
	...
	for en in range(len(ensentence)):
		for hi in range(len(hindisentence)):
			...
			try:
				distance[en][hi] = (1 - probdict[ensentence[en]][hindisentence[hi]])*abs(1.0/(idf_dict[ensentence[en]]+1) - 1.0/(idf_dict[hindisentence[hi]]+1))
				#*(1.0/idf_dict[ensentence[en]]+1)
			except:
				distance[en][hi] = 1
...
def getembeddings(segments, engsentence, hindisentence, mapping, probdict, idf_dict, idf_dict_hi, endict, hidict, outputfile, index):
  ...
  for segment in segments:
	  ...
	  #with codecs.open("/tmp/code_mixed_eval/output_giza_emd_test1.csv",'a','utf-8') as csvfile:
          #with codecs.open("/tmp/output_giza_emd_all.csv",'a','utf-8') as csvfile:
